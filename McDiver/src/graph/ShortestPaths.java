...
public class ShortestPaths<Vertex, Edge> {
    ...
    public void singleSourceDistances(Vertex source) {
        // Implementation: uses Dijkstra's single-source shortest paths algorithm.
        ...
        while(!frontier.isEmpty()) {
            ...
            for(Edge v : graph.outgoingEdges(g)) {
                Vertex u = graph.dest(v);
                if(distances.get(u) == null) {
                    distances.put(u,distances.get(g)+ graph.weight(v));
                    bestEdges.put(u,v);
                    frontier.add(u,distances.get(u));
                }
                else{
                    if(distances.get(g)+ graph.weight(v) < distances.get(u)) {
                        distances.put(u,distances.get(g)+ graph.weight(v));
                        bestEdges.put(u,v);
                        frontier.changePriority(u,graph.weight(v));
                    }
                }
            }
        }
